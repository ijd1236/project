...
def run_app_ml():
    ...
    if st.checkbox('지역별 DDL,GDP 평균',  value=True):
        ...
        ax.bar(mean_GCI.index, mean_GCI.values, color=['blue', 'green', 'yellow', 'red'], alpha=0.8)
        ...
        ax2.bar(mean_GCI.index, mean_GCI.values, color=['blue', 'green', 'yellow', 'red'], alpha=0.8)
